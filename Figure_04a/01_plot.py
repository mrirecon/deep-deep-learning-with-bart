...
def plot(outfile):
    ...
    pattern = cfl.readcfl(os.environ['REPO'] + "/00_data/01_example_data/01_vn_data/pat")[::-1,::-1]
    ...
    axes[0,3].set_title("VarNet (TensorFlow)")
    axes[0,4].imshow(np.abs(out_bart), vmin = 0, vmax=scale, cmap="gray")
    axes[0,4].set_title("VarNet (BART)")
    ...
    axes[1,2].imshow(np.abs(out_pics - ref), vmin = 0, vmax=.1*scale, cmap="gray")
    ...
    axes[1,3].imshow(np.abs(out_tf- ref), vmin = 0, vmax=.1*scale, cmap="gray")
    ...
    axes[1,4].imshow(np.abs(out_bart-ref), vmin = 0, vmax=.1*scale, cmap="gray")
